...
function Oop() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Filter only Semester 2 OOP materials
        const filteredMaterials = data.duMaterials?.filter(
          (item) => item.sem === "2" && (item.subject.toLowerCase() === "oop" || item.subject === "Object Oriented Programming")
        ) || [];
  ...
  return (
    <div className="main-container">
      <Navbar />
      <h1 className="text-center text-3xl font-bold text-gray-800 my-6">OOP - Semester 2</h1>
