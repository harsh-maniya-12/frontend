...
function Pc() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (Array.isArray(response.data.duMaterials)) {
          // Filter for Semester 4 and subject "PC"
          const filteredMaterials = response.data.duMaterials.filter(
            (item) => item.sem === "4" && (item.subject.toLowerCase() === "pc" || item.subject === "Professional Communication - I")
          );
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      <h1 className="text-3xl font-bold text-center text-gray-800 mb-6">Programming Concepts (PC) - Semester 4</h1>
      ...
      {loading ? (
        ...
        <p className="text-lg text-center text-gray-600">No PC materials available for Semester 4.</p>
      )}
    </div>
  );
}
