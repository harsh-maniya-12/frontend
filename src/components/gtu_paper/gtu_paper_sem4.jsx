  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Filter only Semester 4 GTU Papers
        const sem4Materials = (data.duMaterials || []).filter(
          (item) => item.sem === "4" && item.subject && item.gtu_paper && item.gtu_paper_upload?.url
        );

        console.log("Filtered Sem 4 Materials:", sem4Materials);

        // Group materials by subject
        const groupedMaterials = sem4Materials.reduce((acc, item) => {
          if (!acc[item.subject]) {
            acc[item.subject] = [];
          }
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      ...
      {loading ? (
        ...
        <p className="text-lg text-center text-gray-600">No Sem 4 materials available.</p>
      )}
    </div>
  );
}

export default GtuPaperSem4;
