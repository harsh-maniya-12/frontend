  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Filter only Semester 7 GTU Papers
        const sem7Materials = (data.duMaterials || []).filter(
          (item) => item.sem === "7" && item.subject && item.gtu_paper && item.gtu_paper_upload?.url
        );

        console.log("Filtered Sem 7 Materials:", sem7Materials);

        // Group materials by subject
        const groupedMaterials = sem7Materials.reduce((acc, item) => {
          if (!acc[item.subject]) {
            acc[item.subject] = [];
          }
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      ...
      {loading ? (
        ...
        <p className="text-lg text-center text-gray-600">No Sem 7 materials available.</p>
      )}
    </div>
  );
}

export default GtuPaperSem7;
