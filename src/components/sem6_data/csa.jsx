...
function Csa() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (Array.isArray(response.data.duMaterials)) {
          // Filter for Semester 4 and subject "DAA"
          const filteredMaterials = response.data.duMaterials.filter(
            (item) => item.sem === "6" && (item.subject.toLowerCase() === "csa" || item.subject === "Computer System Architecture")
          );
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      <h1 className="text-3xl font-bold text-center text-gray-800 mb-6">CSA - Semester 5</h1>
      ...
      {loading ? (
        ...
        <p className="text-lg text-center text-gray-600">No CSA materials available for Semester 6.</p>
      )}
    </div>
  );
}
