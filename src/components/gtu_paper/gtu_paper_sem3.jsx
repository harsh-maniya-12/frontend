  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Filter only Semester 3 GTU Papers
        const sem3Materials = data.duMaterials?.filter(
          (item) => item.sem === "3" && item.subject && item.gtu_paper && item.gtu_paper_upload?.url
        ) || [];

        console.log("Filtered Sem 3 Materials:", sem3Materials);

        // Group materials by subject
        const groupedMaterials = sem3Materials.reduce((acc, item) => {
          if (!acc[item.subject]) {
            acc[item.subject] = [];
          }
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      ...
      {loading ? (
        ...
        <p className="text-lg text-center text-gray-600">No Sem 3 materials available.</p>
      )}
    </div>
  );
}

export default GtuPaperSem3;
