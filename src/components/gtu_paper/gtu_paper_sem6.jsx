  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Filter only Semester 6 GTU Papers
        const sem4Materials = (data.duMaterials || []).filter(
          (item) => item.sem === "6" && item.subject && item.gtu_paper && item.gtu_paper_upload?.url
        );

        console.log("Filtered Sem 6 Materials:", sem6Materials);

        // Group materials by subject
        const groupedMaterials = sem6Materials.reduce((acc, item) => {
          if (!acc[item.subject]) {
            acc[item.subject] = [];
          }
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      ...
      {loading ? (
        ...
        <p className="text-lg text-center text-gray-600">No Sem 6 materials available.</p>
      )}
    </div>
  );
}

export default GtuPaperSem6;
