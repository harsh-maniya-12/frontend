    ...
    const fetchData = async () => {
      try {
        ...
        if (Array.isArray(response.data.duMaterials)) {
          // Filter for Semester 3 and subject "DBMS2"
          const filteredMaterials = response.data.duMaterials.filter(
            (item) => item.sem === "3" && (item.subject.toLowerCase() === "dbms2" ||item.subject === "Database Management System - II")
          );
