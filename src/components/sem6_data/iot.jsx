...
function Iot() {
      ...
      try {
        ...
        if (Array.isArray(response.data.duMaterials)) {
          ...
          const filteredMaterials = response.data.duMaterials.filter(
            (item) => item.sem === "6" && (item.subject.toLowerCase() === "iot" ||item.subject === "Internet of Things" )
          );
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      <h1 className="text-3xl font-bold text-center text-gray-800 mb-6">IOT - Semester 6</h1>
      ...
      {loading ? (
        ...
        <p className="text-lg text-center text-gray-600">No IOT materials available for Semester 6.</p>
      )}
    </div>
  );
}
