    ...
    const fetchData = async () => {
      try {
        ...
        if (Array.isArray(response.data.duMaterials)) {
          // Filter for Semester 3 and subject "Dmaths"
          const filteredMaterials = response.data.duMaterials.filter(
            (item) => item.sem === "3" && (item.subject.toLowerCase() === "dmaths" || item.subject === "Discrete Mathematics" )
          );
