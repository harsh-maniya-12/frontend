...
function Dm() {
      ...
      try {
        ...
        if (Array.isArray(response.data.duMaterials)) {
          ...
          const filteredMaterials = response.data.duMaterials.filter(
            (item) => item.sem === "4" && (item.subject.toLowerCase() === "dm" || item.subject === "Data Mining")
          );
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      <h1 className="text-3xl font-bold text-center text-gray-800 mb-6">Dm - Semester 4</h1>
      ...
      {loading ? (
        ...
        <p className="text-lg text-center text-gray-600">No DM materials available for Semester 4.</p>
      )}
    </div>
  );
}
